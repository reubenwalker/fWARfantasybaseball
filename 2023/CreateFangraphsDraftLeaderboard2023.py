...
hitters = pd.read_csv('Hitters2023.csv')
...
offense = pd.read_csv('Hitters2023.csv')[['PlayerId', 'Off', 'PA']]
...
pitchers = pd.read_csv('Pitchers2023.csv')
...
fielders = pd.read_csv('Fielders2022.csv')
...
hitter_pos = pd.merge(hitters,pos_elig_final, left_on='PlayerId', right_on='playerid', how='left')
...
leaderboard_final[['WAR', 'hWAR', 'pWAR']] = leaderboard_final[['WAR','hWAR','pWAR']].apply(lambda x: round(x, 1))
leaderboard_final
...
leaderboard_final.to_csv('DraftLeaderboard2023.csv', sep=',')
