...
#Clear leading/trailing space from owner names
...
fielders_of
###Add 2022 Season
fielders2 = pd.read_csv('Fielders2022.csv')
#Need to change LF, CF, RF to OF
fielders2['Pos'][(fielders['Pos'] == 'LF') | (fielders['Pos'] ==  'CF') | (fielders['Pos'] == 'RF')] = 'OF'
#Change playerid to string for later merges:
fielders2['playerid'] = fielders['playerid'].map(str)
#Now we need to combine outfield innings
fielders2_of = fielders.groupby(['playerid','Pos','Name'],as_index=False).sum()
fielders2_of

fielders3 = pd.concat([fielders_of, fielders2_of])
...
eligible_fielders = fielders3[fielders3['Inn'] > 90]
