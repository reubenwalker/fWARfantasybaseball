...
owners = pd.read_csv('Draft and Forget 2023 - Sheet 1.csv')
owners = owners[['PlayerId', 'Owner']].rename(columns={'PlayerId':'playerid'})
mask = owners['Owner'].notna()
owners.loc[mask, 'Owner'] = owners.loc[mask, 'Owner'].apply(lambda x: x.strip())
hitters = pd.read_csv('Hitters2023Final.csv').rename(columns={'PlayerId':'playerid'})
...
pitchers = pd.read_csv('Pitchers2023Final.csv').rename(columns={'PlayerId':'playerid'})
...
fielders_f = pd.read_csv('Fielders2023Final.csv')
fielders_f = fielders_f.rename(columns={'PlayerId':'playerid'})
fielders_0 = pd.read_csv('Fielders2022.csv')
fielders = pd.concat([fielders_f,fielders_0])
#fielders = fielders.rename(columns={'PlayerId':'playerid'})
